...
export default class JsxParser {
    ...
    parseJSX = (jsx: string): JSX.Element | JSX.Element[] | null => {
		const parser = Acorn.Parser.extend(AcornJSX.default({
			autoCloseVoidElements: this.options.autoCloseVoidElements,
		}))
	...
	#parseExpression = (expression: AcornJSX.Expression): any => {
		...
		case 'JSXText':
			const key = this.options.disableKeyGeneration ? undefined : randomHash()
			return this.options.disableFragments
				? expression.value
		...
		case 'CallExpression':
			...
			if (parsedCallee === undefined) {
				this.options.onError!(new Error(`The expression '${expression.callee}' could not be resolved, resulting in an undefined return value.`))
				return undefined
			}
		...
		case 'ExpressionStatement':
			return this.#parseExpression(expression.expression)
		case 'Identifier':
			return (this.options.bindings || {})[expression.name]
		case 'Literal':
	...
	#parseMemberExpression = (expression: AcornJSX.MemberExpression): any => {
		...
		try {
			...
			this.options.onError!(new Error(`Unable to parse ${name}["${path.join('"]["')}"]}`))
		}
	}
	...
	#parseElement = (
		...
		const { allowUnknownElements, components, componentsOnly, onError } = this.options
		...
		const blacklistedAttrs = (this.options.blacklistedAttrs || [])
			.map(attr => (attr instanceof RegExp ? attr : new RegExp(attr, 'i')))
		const blacklistedTags = (this.options.blacklistedTags || [])
		...
		if (name !== '' && !resolvePath(components, name)) {
			if (componentsOnly) {
				...
				return this.options.renderUnrecognized!(name)
			}
		...
		const props: { [key: string]: any } = {
			key: this.options.disableKeyGeneration ? undefined : randomHash(),
		}
