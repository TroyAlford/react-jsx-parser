...
var _acornJsx = __webpack_require__(6);

var _react = __webpack_require__(10);

var _react2 = _interopRequireDefault(_react);

var _parseStyle = __webpack_require__(11);

var _parseStyle2 = _interopRequireDefault(_parseStyle);

var _attributeNames = __webpack_require__(13);

var _attributeNames2 = _interopRequireDefault(_attributeNames);

var _specialTags = __webpack_require__(14);
...
var parserOptions = { plugins: { jsx: true } };
...
var _initialiseProps = function _initialiseProps() {
  ...
  this.handleNewProps = function (props) {
    _this2.blacklistedTags = (props.blacklistedTags || []).map(function (tag) {
      return tag.trim().toLowerCase();
    }).filter(Boolean);
    _this2.blacklistedAttrs = (props.blacklistedAttrs || []).map(function (attr) {
      return attr instanceof RegExp ? attr : new RegExp(attr, 'i');
    });

    var jsx = (props.jsx || '').trim().replace(/<!DOCTYPE([^>]*)>/g, '');
    _this2.ParsedChildren = _this2.parseJSX(jsx);
  };

  this.parseJSX = function (rawJSX) {
    var wrappedJsx = '<root>' + rawJSX + '</root>';
    var parsed = [];
    try {
      parsed = new _acornJsx.Parser(parserOptions, wrappedJsx).parse();
      parsed = parsed.body[0].expression.children || [];
    } catch (error) {
      // eslint-disable-next-line no-console
      if (_this2.props.showWarnings) console.warn(error);
      if (_this2.props.onError) _this2.props.onError(error);
      return [];
    }

    return parsed.map(_this2.parseExpression).filter(Boolean);
  };

  this.parseExpression = function (expression, key) {
    /* eslint-disable no-case-declarations */
    switch (expression.type) {
      case 'JSXElement':
        return _this2.parseElement(expression, key);
      case 'JSXText':
        return expression.value || '';
      case 'JSXAttribute':
        if (expression.value === null) return true;
        return _this2.parseExpression(expression.value);

      case 'ArrayExpression':
        return expression.elements.map(_this2.parseExpression);
      case 'ObjectExpression':
        var object = {};
        expression.properties.forEach(function (prop) {
          object[prop.key.name || prop.key.value] = _this2.parseExpression(prop.value);
        });
        return object;
      case 'JSXExpressionContainer':
        return _this2.parseExpression(expression.expression);
      case 'Literal':
        return expression.value;
  ...
  this.parseElement = function (element, key) {
    var _props = _this2.props,
        _props$bindings = _props.bindings,
        bindings = _props$bindings === undefined ? {} : _props$bindings,
        _props$components = _props.components,
        components = _props$components === undefined ? {} : _props$components;
    var _element$children = element.children,
        children = _element$children === undefined ? [] : _element$children,
        _element$openingEleme = element.openingElement,
        attributes = _element$openingEleme.attributes,
        name = _element$openingEleme.name.name;


    if (/^(html|head|body)$/i.test(name)) return children.map(function (c) {
      return _this2.parseElement(c);
    });

    if (_this2.blacklistedTags.indexOf(name.trim().toLowerCase()) !== -1) return undefined;
    var parsedChildren = void 0;
    if (components[name] || (0, _specialTags.canHaveChildren)(name)) {
      parsedChildren = children.map(_this2.parseExpression);
      if (!components[name] && !(0, _specialTags.canHaveWhitespace)(name)) {
        parsedChildren = parsedChildren.filter(function (child) {
      ...
      if (parsedChildren.length === 0) {
        ...
        var _parsedChildren = parsedChildren;

        var _parsedChildren2 = _slicedToArray(_parsedChildren, 1);

        parsedChildren = _parsedChildren2[0];
      }
    }

    var attrs = _extends({ key: key }, bindings);
    attributes.forEach(function (expr) {
      var rawName = expr.name.name;
      var attributeName = _attributeNames2.default[rawName] || rawName;
      // if the value is null, this is an implicitly "true" prop, such as readOnly
      var value = _this2.parseExpression(expr);

      var matches = _this2.blacklistedAttrs.filter(function (re) {
        return re.test(attributeName);
      });
      if (matches.length === 0) attrs[attributeName] = value;
    });

    if (typeof attrs.style === 'string') {
      attrs.style = (0, _parseStyle2.default)(attrs.style);
    }

    return _react2.default.createElement(components[name] || name, attrs, parsedChildren);
  };

  this.render = function () {
    return _this2.props.renderInWrapper ? _react2.default.createElement(
      'div',
      { className: 'jsx-parser' },
      _this2.ParsedChildren
    ) : _this2.ParsedChildren;
  };
};
...
module.exports = __webpack_require__(7)(__webpack_require__(9));
...
var XHTMLEntities = __webpack_require__(8);
...
var _camelCase = __webpack_require__(12);
